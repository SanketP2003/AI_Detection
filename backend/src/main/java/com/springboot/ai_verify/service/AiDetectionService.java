...
public class AiDetectionService {
    ...
    public Mono<String> detectAiContent(String textToAnalyze) {
        if (mistralApiKey == null || mistralApiKey.isBlank()) {
            log.warn("AI detection attempted but API key is not configured");
        ...
        return webClient.post()
                ...
                .onErrorResume(e -> {
                    log.error("Error calling Mistral API: {}", e.getMessage(), e);
                    if (e instanceof WebClientResponseException wcre &&
                        wcre.getStatusCode() == HttpStatus.UNAUTHORIZED) {
    ...
    private String processMistralResponse(MistralResponse apiResponse) {
        if (apiResponse == null || apiResponse.choices() == null || apiResponse.choices().isEmpty()) {
            log.warn("Mistral API returned empty response");
        ...
        if (firstChoice.message() == null || firstChoice.message().content() == null ||
            ...
            log.warn("Mistral API returned message with no content");
        ...
        if (jsonStart == -1 || jsonEnd == -1 || jsonEnd <= jsonStart) {
            log.error("Failed to find JSON in response: {}", content);
    ...
    private String attemptJsonRepair(String json) {
        ...
        try {
            ...
            log.error("JSON repair failed. Raw: {}", json);
