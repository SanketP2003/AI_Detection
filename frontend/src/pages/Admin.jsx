...
const Admin = () => {
  ...
  const chatSafe = Array.isArray(chat) ? chat : [];
  const sanitizedHistory = historySafe.filter(entry => entry && typeof entry === 'object');
  const sanitizedChat = chatSafe.filter(entry => entry && typeof entry === 'object');

  const chartData = Array.from({ length: 30 }).map((_, idx) => {
    const date = new Date();
    date.setDate(date.getDate() - (29 - idx));
    const label = date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    const dayHistory = sanitizedHistory.filter(h => h?.createdAt && new Date(h.createdAt).toDateString() === date.toDateString());
    const dayChat = sanitizedChat.filter(c => c?.createdAt && new Date(c.createdAt).toDateString() === date.toDateString());
    return { date: label, detector: dayHistory.length, advisor: dayChat.length };
  });

  const detectionData = [
    { name: 'AI-Generated', value: sanitizedHistory.filter(h => h?.result === 'AI').length, color: '#f6c343' },
    { name: 'Human-Written', value: sanitizedHistory.filter(h => h?.result === 'HUMAN').length, color: '#6ed8b5' },
  ];

  const totalDetections = detectionData.reduce((sum, entry) => sum + entry.value, 0);
  const pieData = totalDetections > 0 ? detectionData : [{ name: 'No data', value: 1, color: '#1f2937' }];

  const topUsers = Object.entries([...sanitizedHistory, ...sanitizedChat].reduce((acc, entry) => {
  ...
  const barData = topUsers.length > 0 ? topUsers : [{ username: 'No data', count: 0 }];
            ...
            <div className="grid gap-6">
              <div className="glass-panel border-white/5 p-6">
                ...
                <ResponsiveContainer width="100%" height={360}>
                  <LineChart data={chartData.filter(item => Number.isFinite(item.detector) && Number.isFinite(item.advisor))}>
                    <CartesianGrid stroke="rgba(255,255,255,0.05)" />
                    <XAxis dataKey="date" stroke="#888" />
                    <YAxis stroke="#888" />
                    <Tooltip
                      contentStyle={{ backgroundColor: '#111114', border: '1px solid rgba(255,255,255,0.1)' }}
